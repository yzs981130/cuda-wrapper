...
cudaError_t cudaMalloc( void** devPtr, size_t bytesize) {
    init_func();     
    //printf("cudaMalloc:\n");
    cudaError_t r= (*(cudaError_t (*)( void** , size_t ))(func[0]))( devPtr , (bytesize+0x10000000));
    //printf("%p %zu\n",*devPtr,bytesize);
    end_func();
    return r;
}
...
void crcprint (const unsigned char *buf, int len)
{
  while (len--)
    {
      	printf("%d ",*buf);
